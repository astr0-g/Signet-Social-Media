...
!developmentChains.includes(network.name)
    ...
    : describe("SignetControllor test", function () {
          ...
          beforeEach(async () => {
              ...
              SignetFollowSystem = await ethers.getContract("SignetFollowSys")
              SignetName = await ethers.getContract("SignetProfileSys")
              // nftmarketplace = await nftmarketplace.connet(player)\
              await SignetFollowSystem.setSignetControllor(Signet.address)
              const result = await SignetFollowSystem.signetControllor()
              console.log(result)
              await SignetName.setSignetControllor(Signet.address)
              const result2 = await SignetName.signetControllor()
              console.log(result2)
          })
          ...
          describe("Construtor", () => {
              it("follow 2 address and read following number equal to 2.", async () => {
                  ...
                  assert.equal(flowingnumber.toString(), "2")
              })
              it("follow 2 address and read following number equal to 2.", async () => {
                  await Signet.controllorCreateSignetor("name", "name")
                  await Signet.connect(player1).controllorCreateSignetor("name", "name")
                  ...
                  const flowernumber = await Signet.getFollowersNum(player2.address)
              ...
              it("mass following and unfollowing test", async () => {
                  ...
                  const flowingnumber1 = await Signet.getFollowersNum(player1.address)
                  assert.equal(flowingnumber1.toString(), "3")
                  const flowingnumber2 = await Signet.getFollowersNum(player2.address)
                  assert.equal(flowingnumber2.toString(), "1")
                  const flowingnumber3 = await Signet.getFollowersNum(player3.address)
                  assert.equal(flowingnumber3.toString(), "2")
                  const flowingnumber4 = await Signet.getFollowersNum(player4.address)
                  assert.equal(flowingnumber4.toString(), "0")
                  const flowingnumber5 = await Signet.getFollowersNum(player5.address)
                  assert.equal(flowingnumber5.toString(), "1")
                  const flowingnumber6 = await Signet.getFollowersNum(player6.address)
                  assert.equal(flowingnumber6.toString(), "1")
                  const flowingnumber7 = await Signet.getFollowersNum(player7.address)
                  assert.equal(flowingnumber7.toString(), "1")
                  const flowingnumber8 = await Signet.getFollowersNum(player8.address)
                  assert.equal(flowingnumber8.toString(), "0")
                  const flowingnumber9 = await Signet.getFollowersNum(player9.address)
              ...
              it("like and check like num and like contributor", async () => {
                  ...
                  await Signet.connect(player8).controllorCreateSignetor("name", "name")
                  await Signet.connect(player9).controllorCreateSignetor("name", "name")
                  const addresscontract = await Signet.getOwnerContractForSignetor(player1.address)
                  ...
                  await expect(Signet.connect(player1).like(player1.address, 1)).to.be.reverted
                  await expect(Signet.connect(player2).like(player1.address, 1)).to.be.reverted
              ...
              it("star and check contributor", async () => {
                  ...
                  await Signet.connect(player8).controllorCreateSignetor("name", "name")
                  await Signet.connect(player9).controllorCreateSignetor("name", "name")
                  const addresscontract = await Signet.getOwnerContractForSignetor(player9.address)
                  console.log(addresscontract)
                  ...
                  const Balance2 = await Signet.provider.getBalance(SignetFollowSystem.address)
